...
class TasksTableItem extends Component {
  ...
  render() {
    ...
    if (this.props.showContacts) {
      showContacts = (<td><Select value={task.contactId} items={this.props.contacts} onChange={(event) => this.props.editTaskDu(task.id, task.title, task.description, event.target.value)} bindLabel="firstName" bindLabelSecond="lastName" bindValue="id" /></td>);
    }
    if (task.startDate) {
